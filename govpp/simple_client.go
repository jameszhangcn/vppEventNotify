...
import (
	...
	"git.fd.io/govpp.git/binapi/testgovpp"
)
...
func main() {
	...
	cuupEventNotifications(ch, idx)
	//testgovpp_enable(ch,idx)
...
func testgovpp_enable(ch api.Channel, index interface_types.InterfaceIndex) {
	fmt.Printf("testgovpp_enable enter \n")

	// enable interface events in VPP
	err := ch.SendRequest(&testgovpp.TestgovppEnableDisable{
		SwIfIndex:           0,
		EnableDisable: true,
	}).ReceiveReply(&testgovpp.TestgovppEnableDisableReply{})
	if err != nil {
		logError(err, "enabling testgovpp")
		return
	}
...
func cuupEventNotifications(ch api.Channel, index interface_types.InterfaceIndex) {
	fmt.Printf("Subscribing to notificaiton events for gtp error ind interface index %d\n", index)

	notifChan := make(chan api.Message, 100)

	// subscribe for specific notification message
	sub, err := ch.SubscribeNotification(notifChan, &testgovpp.CuUpGtpErrorIndEvent{})
	if err != nil {
		logError(err, "subscribing to cu up events")
		return
	}

	// enable interface events in VPP
	err = ch.SendRequest(&testgovpp.WantCuUpGtpErrorIndEvents{
		PID:           uint32(os.Getpid()),
		EnableDisable: true,
	}).ReceiveReply(&testgovpp.WantCuUpGtpErrorIndEventsReply{})
	if err != nil {
		logError(err, "enabling cuup gtp error ind events")
		return
	}
	...
	go func() {
		for notif := range notifChan {
			e := notif.(*testgovpp.CuUpGtpErrorIndEvent)
			fmt.Printf("incoming gtp error ind event: %+v\n", e)
			marshal(e)
		}
	}()
    ...
    for {
		...
		fmt.Println("In client main loop ... ")
	}
	// unsubscribe from delivery of the notifications
	err = sub.Unsubscribe()
	if err != nil {
